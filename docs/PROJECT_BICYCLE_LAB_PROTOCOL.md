...
- A text editor for reviewing receipts generated by local runs.
...
Create a clean lab folder and download the standalone package:
...
Confirm the standalone tool body exists:

```powershell
Test-Path ".\San.sln"
Test-Path ".\build\line-manifest.json"
Test-Path ".\src"
Test-Path ".\tests"
```
...
Payload:
Commit or version:
Package source: release zip / tagged clone
