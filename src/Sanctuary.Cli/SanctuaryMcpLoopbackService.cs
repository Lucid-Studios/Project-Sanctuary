...
internal static class SanctuaryMcpLoopbackService
{
    ...
    public static async Task RunAsync(IReadOnlyList<string> args)
    {
        ...
        var startupReceipt = service.Run(new SanctuaryRequest
        {
            ...
            CmeId = defaultCallerCmeId,
            CmeIdentitySelected = true,
            ServiceIdentityId = serviceIdentityId,
            CallerCmeId = defaultCallerCmeId,
            ThreadBindingId = defaultThreadBindingId,
            IdentityTemplateId = defaultIdentityTemplateId,
            SoulFrameId = defaultSoulFrameId,
            AgentiCoreId = defaultAgentiCoreId,
            SubjectCmeId = defaultSubjectCmeId,
            Domain = domain,
        ...
        string jobClass)
    {
        ...
        var effectiveCmeId = ResolveEffectiveCallerCmeId(invocation.CmeId, defaultCmeId);
        var effectiveThreadBindingId = ResolveEffectiveThreadBindingId(invocation.ThreadBindingId, defaultThreadBindingId);
        if (string.IsNullOrWhiteSpace(effectiveCmeId))
        {
            await WriteResponseAsync(stream, 400, BuildCmeIdentityRequiredPayload());
            return;
        }
        if (string.IsNullOrWhiteSpace(effectiveThreadBindingId))
        {
            await WriteResponseAsync(stream, 400, BuildCmeThreadBindingRequiredPayload(effectiveCmeId));
            return;
        }
        ...
        try
        {
            receipt = service.Run(new SanctuaryRequest
            {
                Command = command,
                InstallRootPath = installRoot,
                IntakeRootPath = intakeRoot,
                OperatorName = operatorName,
                CmeId = effectiveCmeId,
                CmeIdentitySelected = true,
                ServiceIdentityId = serviceIdentityId,
                CallerCmeId = effectiveCmeId,
                ThreadBindingId = effectiveThreadBindingId,
                IdentityTemplateId = string.IsNullOrWhiteSpace(invocation.IdentityTemplateId)
                    ? defaultIdentityTemplateId
                    : invocation.IdentityTemplateId,
                SoulFrameId = ResolveEffectiveBodyId(invocation.SoulFrameId, defaultSoulFrameId),
                AgentiCoreId = ResolveEffectiveBodyId(invocation.AgentiCoreId, defaultAgentiCoreId),
                ParentCmeId = invocation.ParentCmeId,
                SubjectCmeId = ResolveEffectiveSubjectCmeId(invocation.SubjectCmeId, defaultSubjectCmeId),
        ...
        {
            await WriteResponseAsync(stream, 403, BuildCmeIdentityDeniedPayload(effectiveCmeId, exception.Message));
            return;
        }
        ...
        if (string.Equals(method, "tools/call", StringComparison.Ordinal))
        {
            ...
            if (string.IsNullOrWhiteSpace(effectiveCmeId))
            {
                return new
                {
                    ...
                    {
                        code = -32602,
                        message = "cme-identity-required",
                        data = BuildCmeIdentityRequiredPayload()
                    }
                };
            }
            if (string.IsNullOrWhiteSpace(effectiveThreadBindingId))
            {
                return new
                {
                    ...
                    {
                        code = -32602,
                        message = "cme-thread-binding-required",
                        data = BuildCmeThreadBindingRequiredPayload(effectiveCmeId)
                    }
                };
            }
            ...
            catch (ArgumentException exception)
            {
                return new
                {
                    ...
                    error = new
                    {
                        ...
                        data = BuildCmeIdentityDeniedPayload(effectiveCmeId, exception.Message)
                    }
                };
            }
    ...
    private static object BuildMcpToolInputSchema() => new
    {
        ...
        required = new[] { "cmeId", "threadBindingId" },
    ...
    private static object BuildHealth(SanctuaryReceipt startupReceipt, McpServiceBinding binding) => new
    {
        ...
        status = "running",
        owner = "Sanctuary.exe",
        serviceIdentityId = startupReceipt.CmeId,
